...
def index(request):
    ...
    most_popular_posts = Post.objects.popular().prefetch_related('author', prefetch_tags)[:5].fetch_with_comments_count()

    fresh_posts = Post.objects.annotate(Count('comments')).order_by('published_at').prefetch_related('author', prefetch_tags)
